...
export class SearchManagedProperties extends React.Component<ISearchManagedPropertiesProps, ISearchManagedPropertiesState> {
     ...
     */
    public async onChange(option?: IComboBoxOption, index?: number, value?: string) {
        let isSortable = null;
     ...
     */
    public async onChangeMulti(option?: IComboBoxOption, index?: number, value?: string) {
        ...
        if (option.key === LOADING_KEY) {
            return (
                <div style={{height: '100%'}}>
                    <Spinner key={option.key} size={SpinnerSize.small}></Spinner>
                </div>
              );
        } else {

...
export default class Slider extends React.Component<ISliderProps, ISliderState> {
    ...
    public renderPortal() {
        /**
         * CUSTOM Portal ... not nice but with React 15 ... vrrrr
         */
        const HTMLElement = this.flickityNode.querySelector('.flickity-slider');
        let innerHTML: string = '';
        for (let i: number = 0; i < (this.props.children as any).length; i++) {
            innerHTML += this.props.children[i].props.children.props.dangerouslySetInnerHTML.__html;
        }
        if (innerHTML !== '') {
    ...
    public render() {
        ...
        if (this.flickityNode) {
            mountNode = this.flickityNode.querySelector('.flickity-slider');
        }
        return (
            <div>

...
import { IRefinementValue, IRefinementResult, ISearchResult, ISearchResults, IRefinementFilter } from '../../../../models/ISearchResult';
...
import SearchRefinersContainer from './components/Refiners/components/SearchRefinersContainer/SearchRefinersContainer';
import SearchPagination from './components/Pagination/SearchPaginationContainer/SearchPaginationContainer';
...
export default class SearchResultsContainer extends React.Component<ISearchResultsContainerProps, ISearchResultsContainerState> {
    ...
    public constructor(props: ISearchResultsContainerProps) {
        ...
        this.state = {
            searchQuery: this.props.queryKeywords? this.props.queryKeywords: '',
            results: {
                QueryKeywords: '',
                RefinementResults: [],
                RelevantResults: []
        ...
        this._onRefinementUpdate = this._onRefinementUpdate.bind(this);
    ...
    public render(): React.ReactElement<ISearchResultsContainerProps> {
        ...
        let renderRefiners: JSX.Element = null;
        ...
        if (this.props.showSearchBox) {
            renderSearchBox = 
                <SearchBoxContainer 
                    ...
                    searchInNewPage= {this.props.searchInNewPage}
                    pageUrl= {this.props.pageUrl}
                    openBehavior= {this.props.openBehavior}
                    queryPathBehavior= {this.props.queryPathBehavior}
        ...
        if (items.RelevantResults.length === 0) {
            ...
            const selectedProperties = (this.props.searchService.selectedProperties) ? this.props.searchService.selectedProperties.join(',') : undefined;
            if (this.props.showRefinements) {
                renderRefiners = 
                    <SearchRefinersContainer 
                        availableRefiners= {this.state.availableRefiners}
                        refinersConfiguration= {this.props.refinersConfiguration}
                        showBlank= {this.props.showBlank}
                        displayMode= {this.props.displayMode}
                        onUpdateFilters= {this._onRefinementUpdate}
                        selectedLayout={this.props.selectedLayoutRefiners}
                        language= {this.props.currentUICultureName}
                        query= {this.state.searchQuery + this.props.searchService.queryTemplate + selectedProperties + this.props.searchService.resultSourceId}
                    />
            }    
            if (this.props.showPaging) {
                renderPagination = 
                    <SearchPagination 
                        paginationInformation={this.state.results.PaginationInformation}
        ...
        if (this.state.searchQuery) {
            try {
                ...
                this.setState({
                    ...
                    availableRefiners: searchResults.RefinementResults,
                    areResultsLoading: false
                });
                ...
                this.setState({
                    areResultsLoading: false,
                    results: results,
                    availableRefiners: results.RefinementResults,
                    hasError: true,
    ...
    public async componentDidUpdate(prevProps: ISearchResultsContainerProps, prevState: ISearchResultsContainerState) {
        ...
        if (!isEqual(this.state, prevState)) {
            ...
            if (this.state.selectedRefinementFilters !== prevState.selectedRefinementFilters) {
                executeSearch = true;
            }
   ...
   */
    ...
    /**
     * Callback function to apply pagination change
     * @param page Page number
     */
    private _onRefinementUpdate(appliedRefiners: IRefinementFilter[]) {
        if (appliedRefiners) {
            this.setState({
                selectedRefinementFilters: appliedRefiners
            });
        }
        this.props.searchService.refinementFilters = appliedRefiners;
    }
    /**
     * Callback function to apply pagination change
     * @param page Page number
     */
    private _onPageUpdate(page: number) {
        if (page) {
            this.setState({
                selectedPage: page
            })
    ...
    private async _getSearchResults(props: ISearchResultsContainerProps, pageNumber?: number): Promise<ISearchResults> {
        ...
        // Get search results
        const searchResults = await props.searchService.search(this.state.searchQuery, pageNumber);

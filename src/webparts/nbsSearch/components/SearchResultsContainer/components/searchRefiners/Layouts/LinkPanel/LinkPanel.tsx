...
import { IRefinementResult, IRefinementValue,RefinementOperator } from '../../../../../../../../models/ISearchResult';
import IRefinerConfiguration from '../../../../../../../../models/IRefinerConfiguration';
import IFilterLayoutProps from '../IFilterLayoutProps';
import { isEqual } from '@microsoft/sp-lodash-subset';
import { Icon } from 'office-ui-fabric-react/lib/Icon';
import { Label as TextUI } from 'office-ui-fabric-react/lib/Label';
export default class LinkPanel extends React.Component<ILinkPanelProps, ILinkPanelState> {
  ...
  public render(): React.ReactElement<ILinkPanelProps> {
    const renderSelectedFilterValues: JSX.Element[] = this.props.selectedFilterValues.map((value) => {
      ...
      let betterValue: string = value.RefinementValue;
      if (value.RefinementValue.indexOf('\n\n') !== -1) {
          betterValue = value.RefinementValue.split('\n\n')[0].trim();
      }
      // Date refiner value
      if (/range\(((-?(?:[1-9][0-9]*)?[0-9]{4})-(1[0-2]|0[1-9])-(3[01]|0[1-9]|[12][0-9])T(2[0-3]|[01][0-9]):([0-5][0-9]):([0-5][0-9])(.[0-9]+)?(Z)?).+\)/.test(value.RefinementToken)
        ...
        filterName = `[${filterName}: "${betterValue}"]`;
      }
  ...
  private _onRenderHeader(props: IGroupDividerProps): JSX.Element {
    ...
    return (
      <div className={styles.linkPanelLayout__filterPanel__body__group__header}
        ...
        <TextUI>{props.group.name}</TextUI>
      </div>
    );
  }
  ...
  private _initItems(props: IFilterLayoutProps, refinementValueToRemove?: IRefinementValue): void {
    ...
    let valueToremoveMine:IRefinementValue = null;
    ...
    props.refinementResults.map((refinementResult, i) => {
      ...
      if (refinementValueToRemove) {
        if (refinementResult.Values.filter(value => {
          ...
          valueToremoveMine = refinementValueToRemove;
        }
      }
      if (valueToRemove) {
        newFilterValues = selectedFilterValues.filter((elt) => {
          return elt.RefinementValue !== valueToRemove.RefinementValue;
        });
      }
      items.push(
        <TemplateRenderer
    ...
    this.setState({
      ...
      this._groupedList.forceUpdate();
      if (refinementValueToRemove) {
        this.props.onFilterValuesUpdated(valueToremoveMine.RefinementName, newFilterValues, RefinementOperator.AND);
      }
    });   
  }
}
